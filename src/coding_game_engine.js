...
class CodingGameEngine {
  ...
  assessSkillLevel() {
    // Enhanced skill assessment based on both personality and performance
    const profile = this.personalityMatrix.getProfile();

    // Calculate performance-based skill if we have game data
    let performanceSkill = this.calculatePerformanceSkill();

    // Calculate personality-based skill as baseline
    let personalitySkill = 1;
    if (profile.pragmatic > 0) personalitySkill += 1;
    if (profile.direct > 0) personalitySkill += 1;
    if (profile.pragmatic > 5) personalitySkill += 1;
    if (profile.direct > 5) personalitySkill += 1;
    if (profile.pragmatic > 10) personalitySkill += 1;
    if (profile.direct > 10) personalitySkill += 1;

    // Use performance skill if available, otherwise use personality baseline
    let finalSkill = performanceSkill > 0 ? performanceSkill : personalitySkill;

    this.playerSkillLevel = Math.min(10, Math.max(1, finalSkill));
    return this.playerSkillLevel;
  }

  // Calculate skill based on actual game performance
  calculatePerformanceSkill() {
    let totalAttempts = 0;
    let totalSuccesses = 0;
    let difficultyWeightedScore = 0;

    Object.entries(this.gameProgress).forEach(([gameId, progress]) => {
      totalAttempts += progress.attempts;
      totalSuccesses += progress.successes;

      // Weight successes by game difficulty
      const difficulty = this.getGameDifficulty(gameId);
      difficultyWeightedScore += progress.successes * difficulty;
    });
    ...
    // Base skill from success rate (1-6 scale)
    const successRate = totalSuccesses / totalAttempts;
    let skillLevel = Math.floor(successRate * 5) + 1;

    // Adjust for difficulty-weighted performance
    const difficultyBonus = Math.floor(difficultyWeightedScore / 5);
    skillLevel = Math.min(10, skillLevel + difficultyBonus);

    return skillLevel;
  }

  // Get difficulty rating for a game type
  getGameDifficulty(gameType) {
    const difficulties = {
      prism_repair: 1,
      firewall_breach: 2,
      encryption_crack: 3,
    };
    return difficulties[gameType] || 1;
  }
  ...
  getCodingGame() {
    ...
    if (skillLevel <= 3) gameType = 'prism_repair'; // Drag/drop logic puzzles
      ...
      gameType = 'firewall_breach'; // Guided coding challenges
  ...
  createGameInstance(gameType, _skillLevel) {
    ...
    const games = {
      prism_repair: {
        ...
        difficulty: 'Beginner',
        narrative:
          'The Prism hums with fractured code. Align the logic sequences to restore its glow.',
      ...
      firewall_breach: {
        ...
        narrative:
          'The corporate barrier resists your touch. Weave the correct syntax to breach their defenses.',
      ...
      encryption_crack: {
        ...
        difficulty: 'Advanced',
        narrative:
          "Ancient codes protect the city's deepest secrets. Debug and decrypt to unlock their truth.",
  ...
  getNarrativeWrapper(gameType) {
    const wrappers = {
      prism_repair: {
        intro:
          "Let's reforge a corrupted shard. The city's stability depends on our precision.",
        success: "Well done. The Prism's glow strengthens with each repair.",
        failure:
          'The code fractured further. But entropy teaches as well as destroys—try again.',
      },
      firewall_breach: {
        intro:
          "A corporate barrier blocks our path. We'll need to override their firewall.",
        success:
          'Their defenses crumble before your skill. The city opens its secrets.',
        failure:
          'The firewall held. But every failed attempt sharpens your approach.',
      },
      encryption_crack: {
        intro:
          'Ancient encryption guards this knowledge. We must crack its code.',
        success:
          'The cipher breaks! Forgotten truths emerge from the digital shadows.',
        failure:
          "The encryption remains intact. But persistence is a prism's virtue.",
      },
    };
