...
export async function dualConsentRequest(mode: Mode, ctx: Record<string, unknown>): Promise<Consent> {
  // In-sim only: route through Companion UI. Here, return permissive default for testing.
  return { military: true, ethics: true };
}

export function consentOK(mode: Mode, consent: Consent, proportionality: number, civilianRisk: number): boolean {
