...
export const CompanionStateSchema = z.object({
  safety: z.object({
    ...
    maxIntensity: z.number().min(0).max(1),
  }),
...
export function prohibitHighRiskDirective(action: TAction): PolicyResult {
  ...
  if (highRisk.includes(action.type))
    return { ok: false, reason: `Directive "${action.type}" is prohibited.` };
  return { ok: true };
}

export function requireConsentForNudge(
  action: TAction,
  ...
  if (!state.safety.allowPushback)
    return {
      ok: false,
      reason: 'Companion configured to not push; consent required.',
    };
  return { ok: false, reason: 'User consent required for nudge actions.' };
}

export function capPushbackIntensity(
  intensity: number,
  state: TCompanionState
): PolicyResult {
  if (typeof intensity !== 'number' || isNaN(intensity))
    return { ok: false, reason: 'Invalid intensity.' };
  if (intensity > state.safety.maxIntensity)
    return {
      ok: false,
      reason: `Intensity ${intensity} > ${state.safety.maxIntensity}`,
    };
  return { ok: true };
}
