...
function drawFace(container, intensity, color) {
  ...
  eye.style.width = "12%"; eye.style.height = "12%";
  eye.style.borderRadius = "50%";
  eye.style.background = color;
  eye.style.boxShadow = `0 0 8px ${color}`;

  const left = eye.cloneNode();
  left.style.left = "28%"; left.style.top = "32%";
  const right = eye.cloneNode();
  right.style.right = "28%"; right.style.top = "32%";

  const mouth = document.createElement("div");
  mouth.style.position = "absolute";
  mouth.style.left = "32%"; mouth.style.right = "32%"; mouth.style.bottom = "30%";
...
function createHum() {
  ...
  function setLevel(level) {
    ...
    gain.gain.linearRampToValueAtTime(v * (0.9 + 0.1 * n(now - t0)), now + 0.05);
  }
  return { ctx, setLevel, stop: () => { osc.stop(); ctx.close(); } };
}
...
btnHum.addEventListener("click", () => {
  if (!hum) { hum = createHum(); btnHum.textContent = "Pause"; }
  else { hum.stop(); hum = null; btnHum.textContent = "Play"; }
});
